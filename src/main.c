...
unsigned int get_sub_shader(const char* f, int type){
	unsigned int data = get_file_contents(f);
	const char* c_ptr = data;

	unsigned int id = glCreateShader(type);

	glShaderSource(id, 1, &c_ptr, NULL);
	glCompileShader(id);
...
unsigned int get_vao(){
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, 6 * sizeof(unsigned int), &inds[0], GL_STATIC_DRAW);
...
void setup_render(){
	shader = get_shader("res/test.v", "res/test.f");
	...
	float x_offset = 250.f;
	float y_offset = 250.f;
	...
	for(int i=0;i<16;++i){
		...
		mat4x4_translate(models[i], (125.f * x) + x_offset, (125.f * y) + y_offset, 1.f);
		mat4x4_scale_aniso(models[i], models[i], 100.f, 100.f, 1.f);
	}
}
...
void set_m4x(unsigned int shader, const char* uniform, mat4x4* arrays, unsigned int count){
